...
# we recommend keeping all your units in SI if possible.
...
class MotorPosition(Topic):
    """
    topic that publishes the individual motor positions in ticks from the roboclaw.
    """
...
class RobotRelativeVelocity(Topic):
    """
    Based on the messages from the @MotorVelocity Topic, we compute the
    robot relative "twist" velocity and publish it.
    """
...
class Odometry(Topic):
    """
    Using the robot twist velocity as well as information from the system time topic (provided by the scheduler),
    will compute the current pose estimate from the origin of the robot.
    """
...
class RoboclawWriter(Subscriber):
    """
    Subscriber that takes in motor velocities and writes them via serial to the roboclaw.
    """
...
class setMotorVelocityCommand(Command):
    """
    Command that utilizes the motorController subscriber and writes the velocity.
    """
...
class goToPoseMotionProfile(Command):
    """
    Command that moves the robot to a desired x, y, theta position using odometry, pid and motion profiling.
    """
...
class goToPose(Command):
    """
    Same as the previous but does not utilize a motion profile -- will have more wheel slip.
    """
...
def main():
    ...
    command = generate_mp_gtp(1.5, 0, np.radians(0))
    command.setNext(generate_mp_gtp(0, 0, np.radians(0)))
    ...
    if not is_running_on_robot:
        # if we arent running on the robot, generate a graph.
